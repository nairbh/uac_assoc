...
export const usePermissions = () => {
  ...
  // Vérifier si l'utilisateur est l'utilisateur de test
  const isTestUser = useMemo(() => {
    return user?.id === 'test-admin-user';
  }, [user]);
  ...
  useEffect(() => {
    const loadPermissions = async () => {
      ...
      // Si c'est un utilisateur de test admin, lui donner toutes les permissions
      if (isTestUser) {
        setPermissions(Object.keys(allPermissions) as Permission[]);
        setLoading(false);
        return;
      }
    ...
    loadPermissions();
  }, [user, profile, isTestUser, allPermissions]);

  // Vérifier si l'utilisateur a une permission spécifique
  const hasPermission = (permission: Permission): boolean => {
    if (!user || !profile) return false;
    if (isAdmin || isTestUser) return true; // Les admins et utilisateurs test ont toutes les permissions
  ...
  const hasRole = (requiredRole: Role): boolean => {
    if (!user || !profile) return false;
    if (isTestUser) return true; // L'utilisateur test est considéré comme ayant tous les rôles
