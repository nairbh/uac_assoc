...
-- Insérer quelques dons d'exemple pour le test
INSERT INTO public.donations (user_id, amount, payment_method, donation_type, notes) VALUES
((SELECT id FROM public.profiles WHERE email = 'admin@atmf-argenteuil.org'), 50.00, 'card', 'general', 'Don mensuel'),
((SELECT id FROM public.profiles WHERE email = 'admin@atmf-argenteuil.org'), 100.00, 'bank_transfer', 'event', 'Soutien événement Ramadan'),
((SELECT id FROM public.profiles WHERE email = 'admin@atmf-argenteuil.org'), 25.00, 'card', 'project', 'Aide aux familles');
